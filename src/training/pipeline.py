...
from model_loading.normalizations import get_normal_stats
...
class LearnerConfig:
    ...
    def from_dict(cls, config: Mapping[str, Any] | None) -> "LearnerConfig":
        ...
        return cls(
            ...
            valid_eval_freq=_step_interval(data.get("valid_eval_freq", data.get("eval_every_steps"))),
            logging_eval_freq=_step_interval(data.get("logging_eval_freq", data.get("log_every_steps"))),
        )
...
def _resolve_step_frequency(value: Any, *, default: int, name: str) -> int:
...
def parse_batch(batch: Any) -> Batch:
    ...
    if isinstance(batch, Mapping):
        ...
        explicit_metadata = batch.get("metadata")
        metadata = (
            dict(explicit_metadata)
...
class TorchLearner:
    ...
    def fit(
        ...
        *,
        ...
        set_seed(seed)
        history: dict[str, Any] = {"train": [], "train_batch": [], "valid": {}}
        start = perf_counter()
        recent_losses: list[float] = []
        run_step = 0
        steps_per_epoch = _safe_len(train_loader)
        total_steps = None if steps_per_epoch is None else int(epochs) * steps_per_epoch
        default_interval = max(1, steps_per_epoch or 1)
        eval_value = valid_eval_freq if valid_eval_freq is not None else eval_every_steps
        log_value = logging_eval_freq if logging_eval_freq is not None else log_every_steps
        valid_interval = _resolve_step_frequency(
            eval_value,
            default=default_interval,
            name="valid_eval_freq",
        )
        logging_interval = _resolve_step_frequency(
            log_value,
            default=default_interval,
        ...
        if logger is not None:
            logger.info(
                "training epochs=%s steps_per_epoch=%s total_steps=%s valid_eval_freq=%s logging_eval_freq=%s",
                epochs,
        ...
        criterion_name = getattr(self.criterion, "name", "loss")

        def record_train_batch() -> dict[str, Any] | None:
            ...
            result = {
                ...
                "loss": loss_value,
                "losses": {criterion_name: loss_value},
            }
            history["train_batch"].append(result)
            recent_losses.clear()
            return result

        def run_validation(*, log: bool = False) -> dict[str, dict[str, Any]]:
        ...
        for epoch in range(int(epochs)):
            ...
            for batch in train_loader:
                ...
                should_valid_eval = bool(
                    run_step % valid_interval == 0
                )
                should_logging_eval = run_step % logging_interval == 0
                if should_valid_eval:
                    train_batch_result = record_train_batch()
                    if should_logging_eval and train_batch_result is not None and logger is not None:
                        logger.info(
                            "step=%s train_batch=%s",
                            self.global_step,
                            train_batch_result.get("losses", {}),
                        )
                    run_validation(log=should_logging_eval)
        ...
        if recent_losses:
            train_batch_result = record_train_batch()
            if train_batch_result is not None and logger is not None:
                logger.info(
                    "step=%s train_interval=%s",
                    self.global_step,
                    train_batch_result.get("losses", {}),
                )
            run_validation(log=True)
