"""Curriculum-aware window sampling, datasets, and collation."""
...
class IndexSampler:
    ...
    def _validate(self) -> None:
        ...
        if cfg.individual_weights is not None:
            weights = np.asarray(cfg.individual_weights, dtype=float)
            if weights.shape != (self.individuals,):
                raise ValueError(
                    "individual_weights must contain one value per local individual"
                )
            if not np.isfinite(weights).all() or np.any(weights < 0):
                raise ValueError("individual_weights must be finite and non-negative")
            candidates = self._raw_individual_candidates()
            if weights[candidates].sum() <= 0:
                raise ValueError("candidate individual weights must have positive sum")

    def _individual_probabilities(
        self,
        candidates: Sequence[int],
    ) -> Optional[np.ndarray]:
        if self.config.individual_weights is None:
            return None
        weights = np.asarray(self.config.individual_weights, dtype=float)[
            list(candidates)
        ]
    ...
    def _sample_individuals(self, fixed: Optional[int] = None) -> List[int]:
        ...
        return np.random.choice(
            ...
            size=count,
            replace=False,
            p=self._individual_probabilities(candidates),
        ).astype(int).tolist()
